...
class EnvLite:
    def __init__(self, X: np.ndarray, Info: np.ndarray, batch_size: int = 64, seed: int = 7):
    ...
    def _row_normalize(probs: np.ndarray) -> np.ndarray:
        ...
        out = np.divide(probs, s, out=np.zeros_like(probs, dtype=np.float32), where=(s > 0))
        return out

    def step(self, action: np.ndarray) -> np.ndarray:
        assert self.X_b is not None, "Call reset() before step()."
        assert action.shape == (self.batch_size, self.V), f"action must be {(self.batch_size, self.V)}"
        ...
        reward = ((self.rebate_b / 10000.0) * filled + self.punish_b * unfilled).sum(axis=1)
...
if __name__ == "__main__":
    ...
    action = np.full((env.batch_size, env.V), 1.0 / env.V, dtype=np.float32) # function of X_b
